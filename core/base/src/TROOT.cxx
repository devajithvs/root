...
void TROOT::InitInterpreter()
{
   ...
   if (!dlsym(RTLD_DEFAULT, "usedToIdentifyRootClingByDlSym")
      ...
      void *libacppHandle = dlopen("/your/desired/install/location/lib/libacpp-rt.so", RTLD_LAZY | RTLD_GLOBAL);
      if (!libacppHandle) {
      ...
      void *libabackendHandle = dlopen("/your/desired/install/location/lib/hipSYCL/librt-backend-cuda.so", RTLD_NOW);
      if (!libabackendHandle) {
         fprintf(stderr, "Loading libabackendHandle failed\n");
         TString err = dlerror();
         fprintf(stderr, "Fatal in <TROOT::InitInterpreter>: cannot load library %s\n", err.Data());
         exit(1);
      }
