...
//   PMBuilder.populateFunctionPassManager(*m_FPM[OptLevel]);
  ...
  nPB.registerModuleAnalyses(m_MAM[OptLevel]);
  nPB.registerCGSCCAnalyses(m_CGAM[OptLevel]);
  nPB.registerFunctionAnalyses(m_FAM[OptLevel]);
  nPB.registerLoopAnalyses(m_LAM[OptLevel]);
  nPB.crossRegisterProxies(m_LAM[OptLevel], m_FAM[OptLevel],
                           m_CGAM[OptLevel], m_MAM[OptLevel]);

  auto plugin = PassPlugin::Load("/usr/local/lib/libacpp-clang.so");

  if (plugin) {
    plugin.get().registerPassBuilderCallbacks(nPB);
  }
  ...
  m_nMPM[OptLevel] = nPB.buildPerModuleDefaultPipeline(level);
}
...
void BackendPasses::runOnModule(Module& M, int OptLevel) {
  ...
  m_nMPM[OptLevel].run(M, m_MAM[OptLevel]);
}
