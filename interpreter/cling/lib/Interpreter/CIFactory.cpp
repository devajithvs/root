...
#undef DUMP_BOOLEAN
  ...
  // Goal is to make this as close to the same function in
  ...
  createCIImpl(std::unique_ptr<llvm::MemoryBuffer> Buffer,
    ...
    // // add prefix to diagnostic messages if second compiler instance is
    // existing
    // // e.g. in CUDA mode
    // std::string ExeName = COpts.CUDAHost ? "cling" :
    //                   COpts.CUDADevice ? "cling-ptx" : "";
    // auto DiagnosticPrinter =
    // std::make_unique<TextDiagnosticPrinter>(cling::errs(), &DiagOpts);
    // DiagnosticPrinter->setPrefix(ExeName);
    // // Transfer ownership of the client to DiagnosticsEngine
    // Diags.setClient(DiagnosticPrinter.release(), /*Owns it*/ true);
...
CIFactory::create(std::vector<const char *> &ClangArgv) {
  ...
  auto ErrOrCC1Args = GetCC1Arguments(Compilation.get(), &Diags);
