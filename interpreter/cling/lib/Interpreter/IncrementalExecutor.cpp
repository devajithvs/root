...
/// This is used by `IncrementalExecutor::getGenerator()` to yield a generator that
/// resolves symbols defined in the IncrementalExecutor object on which the function
/// is called, which in turn may be used to provide lookup across different
/// IncrementalExecutor instances.
...
{
  m_Callbacks = nullptr;
  m_DyLibManager.initializeDyld([](llvm::StringRef) { /*ignore*/
                                                      return false;
  });

  // MSVC doesn't support m_AtExitFuncsSpinLock=ATOMIC_FLAG_INIT; in the class
  // definition
  ...
  auto&& ErrorReporter = [this, LinkerPrefix, Verbose](Error Err) {
    Err = handleErrors(std::move(Err),
                       [&](std::unique_ptr<SymbolsNotFound> Err) -> Error {
                         ...
                         for (auto&& symbol : Err->getSymbols()) {
                           ...
                           HandleMissingFunction(symbolStr);
                         }
  ...
  m_BackendPasses.reset(new BackendPasses(
      CI.getCodeGenOpts(), *Jit, m_CompiledModules, getTargetMachine()));
}
...
void IncrementalExecutor::replaceSymbol(const char* Name, void* Addr) {
  assert(Addr);
  // FIXME: Look at the registration of at_quick_exit and uncomment.
  // assert(getSymbolAddress(Name, /*IncludeHostSymbols*/true) &&
  //        "The symbol must exist");
  addOrReplaceDefinition(Name, llvm::pointerToJITTargetAddress(Addr));
}
...
void* IncrementalExecutor::getAddressOfGlobal(llvm::StringRef symbolName,
  ...
  void* address = getSymbolAddress(symbolName, includeHostSymbols);
...
IncrementalExecutor::getPointerToGlobalFromJIT(llvm::StringRef name) const {
  ...
  void* addr = getSymbolAddress(name, false /*no dlsym*/);
...
std::unique_ptr<llvm::orc::DefinitionGenerator> IncrementalExecutor::getGenerator() {
  return std::make_unique<DelegateGenerator>(
      [&](StringRef UnmangledName) { return Jit->lookup(UnmangledName); });
}

void IncrementalExecutor::addModule(Transaction& T) {
  ...
  if (Error Err = Jit->addIRModule(RT, std::move(TSM))) {
    logAllUnhandledErrors(std::move(Err), errs(),
                          "[IncrementalExecutor] addModule() failed: ");
    return;
  }
}

llvm::Error IncrementalExecutor::removeModule(const Transaction& T) {
...
JITTargetAddress
IncrementalExecutor::addOrReplaceDefinition(StringRef Name,
  ...
  if (Error Err = DyLib.define(absoluteSymbols({*It}))) {
    logAllUnhandledErrors(std::move(Err), errs(),
                          "[IncrementalExecutor] define() failed: ");
...
void* IncrementalExecutor::getSymbolAddress(StringRef Name, bool IncludeHostSymbols) const {
...
bool IncrementalExecutor::doesSymbolAlreadyExist(StringRef UnmangledName) {
