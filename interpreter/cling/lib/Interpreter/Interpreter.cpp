...
namespace cling {
  ...
  llvm::Error Interpreter::CreateExecutor() {
    const clang::TargetInfo &TI =
        getCI()->getASTContext().getTargetInfo();
  ...
  Interpreter::Interpreter(int argc, const char* const *argv,
    ...
    if (!isInSyntaxOnlyMode() && !m_Opts.CompilerOpts.CUDADevice) {
      for (const llvm::StringRef& Sym : Syms) {
        llvm::Expected<llvm::orc::ExecutorAddr> Addr = m_Executor->getSymbolAddress(Sym, false /*no dlsym*/);
#if defined(__linux__)
        // We need to look for the mangled name of at_quick_exit on linux.
        if (!Addr && Sym.equals("at_quick_exit"))
          Addr = m_Executor->getSymbolAddress("_Z13at_quick_exitPFvvE", false /*no dlsym*/);
#endif
        if (!Addr) {
  ...
  Interpreter::Interpreter(const Interpreter &parentInterpreter, int argc,
    ...
    if (CompilerInstance* CI = getCIOrNull()) {
      ...
      // parent Interpreter.
      auto m_ExecutorParent = parentInterpreter.m_Executor;
      m_Executor->GetExecutionEngine.getMainJITDylib().addGenerator(std::make_unique<DelegateGenerator>([&](StringRef UnmangledName) { return m_ExecutorParent->GetExecutionEngine.lookup(UnmangledName); }));
...
// #endif
  ...
  IncrementalExecutor::jitInitOrWrapper(llvm::StringRef funcname, void (*&fun)(void*)) const {
    llvm::Expected<llvm::orc::ExecutorAddr> fun_ptr = m_Executor->getSymbolAddress(funcname, false /*no dlsym*/); 

    // check if there is any unresolved symbol in the list
    if (!fun_ptr)
      return kExeUnresolvedSymbols;

    fun = reinterpret_cast<void (*)(void*)>(fun_ptr);
    return kExeSuccess;
  }
  ...
  Interpreter::compileFunction(llvm::StringRef name, llvm::StringRef code,
    ...
    return jitTargetAddressToPointer<void*>(m_Executor->getSymbolAddress(name, false /*no dlsym*/)->getValue());
  }
  ...
  Interpreter::addGenerator(std::unique_ptr<llvm::orc::DefinitionGenerator> G) {
    if (m_Executor)
      m_Executor->GetExecutionEngine.getMainJITDylib().addGenerator(std::move(G));
  }
