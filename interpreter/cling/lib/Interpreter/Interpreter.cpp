...
namespace cling {
  ...
  Transaction* Interpreter::Initialize(bool NoRuntime, bool SyntaxOnly) {
    ...
    Strm
        << "#ifdef __cplusplus\n"
           ...
           "void __cling_Interpreter_SetValueNoAlloc(void*, void*, void*, "
           "double);\n"
           "void __cling_Interpreter_SetValueNoAlloc(void*, void*, void*, long "
           "double);\n"
           "void "
           "__cling_Interpreter_SetValueNoAlloc(void*,void*,void*,unsigned "
           "long long);\n"
           "template <class T, class = T (*)() /*disable for arrays*/>\n"
           "void __cling_Interpreter_SetValueCopyArr(T* Src, void* Placement, "
           "unsigned long Size) {\n"
           "for (auto Idx = 0; Idx < Size; ++Idx)\n"
           "new ((void*)(((T*)Placement) + Idx)) T(Src[Idx]);\n"
           "}\n"
           "template <class T, unsigned long N>\n"
           "void __cling_Interpreter_SetValueCopyArr(const T (*Src)[N], void* "
  ...
  namespace {
    ...
    public:
      RuntimeInterfaceBuilder(Interpreter& In, ASTContext& C, Sema& SemaRef,
      ...
      ExprResult getCall() {
        ...
        Expr* TypeArg = cling::utils::Synthesize::CStyleCastPtrExpr(
            &S, Ctx.VoidPtrTy, (uintptr_t)Ty.getAsOpaquePtr());
        // The QualType parameter `OpaqueType`, represented as `void*`.
        Args.push_back(TypeArg);

        // We push the last parameter based on the type of the Expr. Note we
        // need special care for rvalue struct.
        ...
        switch (Kind) {
          ...
          case Interpreter::InterfaceKind::CopyArray: {
            ...
            if (Kind == Interpreter::InterfaceKind::CopyArray) {
              ...
              Expr* ArrSizeExpr =
                  cling::utils::Synthesize::IntegerLiteralExpr(Ctx, ArrSize);
            ...
            ExprResult CXXNewCall =
                S.BuildCXXNew(E->getSourceRange(),
                              ...
                              /*TypeIdParens=*/SourceRange(), TSI->getType(),
                              TSI, std::nullopt, E->getSourceRange(), E);
          ...
          case Interpreter::InterfaceKind::NoAlloc: {
            ...
            return S.ActOnCallExpr(
                /*Scope=*/nullptr,
                Interp.getValuePrintingInfo()
                    [Interpreter::InterfaceKind::NoAlloc],
                E->getBeginLoc(), Args, E->getEndLoc());
          }
          default: llvm_unreachable("Unhandled Interpreter::InterfaceKind");
        }
      }
      ...
      Interpreter::InterfaceKind VisitReferenceType(const ReferenceType* Ty) {
        ExprResult AddrOfE =
            S.CreateBuiltinUnaryOp(SourceLocation(), UO_AddrOf, E);
    ...
    private:
      ...
      void HandleIntegralOrEnumType(const Type* Ty) {
        TypeSourceInfo* TSI =
            Ctx.getTrivialTypeSourceInfo(Ctx.UnsignedLongLongTy);
    ...
    namespace internal {
      ...
      Value EvaluateDynamicExpression(Interpreter* interp, DynamicExprInfo* DEI,
        ...
        if (!ret.isValid()) {
          ...
          CompilationException::throwingHandler(nullptr,
                                                (msg + DEI->getExpr()).c_str(),
                                                false /*backtrace*/);
        }
        return ret;
      }
...
CLING_LIB_EXPORT void* __cling_Interpreter_SetValueWithAlloc(void* This,
                                                             void* OutVal,
                                                             void* OpaqueType) {
  cling::Value& VRef = *(cling::Value*)OutVal;
  VRef = cling::Value(clang::QualType::getFromOpaquePtr(OpaqueType),
                      *static_cast<cling::Interpreter*>(This));
  return VRef.getPtr();
}

// Pointers, lvalue struct that can take as a reference.
CLING_LIB_EXPORT void __cling_Interpreter_SetValueNoAlloc(void* This,
                                                          void* OutVal,
                                                          void* OpaqueType,
                                                          void* Val) {
  cling::Value& VRef = *(cling::Value*)OutVal;
...
static void SetValueDataBasedOnQualType(cling::Value& V,
...
#define X(type, name)                                                          \
  case BuiltinType::name: V.set##name(Data); break;
