...
namespace cling {
  ...
  static QualType findBuiltinType(llvm::StringRef typeName, ASTContext &Context)
  {
    ...
    if (!issigned && !isunsigned) {
      if (typeName == "bool") return Context.BoolTy;
      if (typeName == "float") return Context.FloatTy;
      if (typeName == "double") return Context.DoubleTy;
      if (typeName == "long double") return Context.LongDoubleTy;

      if (typeName == "wchar_t") return Context.WCharTy;
      if (typeName == "char16_t") return Context.Char16Ty;
      if (typeName == "char32_t") return Context.Char32Ty;
    }
    /* Missing
