...
namespace llvm {
// ID is unique in LazySpecializationInfo, it is redundant to calculate IsPartial.
template <> struct DenseMapInfo<clang::serialization::reader::LazySpecializationInfo> {
  using LazySpecializationInfo = clang::serialization::reader::LazySpecializationInfo;
  using Wrapped = DenseMapInfo<clang::serialization::DeclID>;

  static inline LazySpecializationInfo getEmptyKey() {
    return {Wrapped::getEmptyKey(), false};
  }

  static inline LazySpecializationInfo getTombstoneKey() {
    return {Wrapped::getTombstoneKey(), false};
  }

  static unsigned getHashValue(const LazySpecializationInfo &Key) {
    return Wrapped::getHashValue(Key.ID);
  }
