...
public:
   ...
   */
   LorentzVector(const Scalar &a, const Scalar &b, const Scalar &c, const Scalar &d) : fCoordinates(a, b, c, d) {}
   ...
   */
   ...
   explicit LorentzVector(const LorentzVector<Coords> &v) : fCoordinates(v.Coordinates())
   {
   }
   ...
   */
   template <class ForeignLorentzVector,
             typename = decltype(std::declval<ForeignLorentzVector>().x() + std::declval<ForeignLorentzVector>().y() +
                                 std::declval<ForeignLorentzVector>().z() + std::declval<ForeignLorentzVector>().t())>
   explicit LorentzVector(const ForeignLorentzVector &v) : fCoordinates(PxPyPzE4D<Scalar>(v.x(), v.y(), v.z(), v.t()))
   {
   }
   ...
   */
   ...
   /**
      Exact equality
   */
   bool operator==(const LorentzVector &rhs) const { return fCoordinates == rhs.fCoordinates; }
   bool operator!=(const LorentzVector &rhs) const { return !(operator==(rhs)); }
   ...
   */
   ...
   LorentzVector operator+() const { return *this; }
...
operator<<(std::basic_ostream<char_t, traits_t> &os, LorentzVector<Coords> const &v)
{
   ...
   if (detail::get_manip(os, detail::bitforbit)) {
      ...
      os << detail::get_manip(os, detail::open) << a << detail::get_manip(os, detail::sep) << b
         << detail::get_manip(os, detail::sep) << c << detail::get_manip(os, detail::sep) << d
